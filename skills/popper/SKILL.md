...
| `enable` | 포그라운드 출력 | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/popper_plugin.py" enable --grant` | 명시 요청 시 CLAUDE.md에 소유 @import 한 줄 추가 |
| `rollback` | 포그라운드 출력 | `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/popper_plugin.py" rollback` | Popper가 추가한 @import 한 줄만 제거 |
